...
public:
     ...
     *
	 * @param[in]  clean_flag	 Allow to clean all bonding information
     ...
     * @return BLE_ERROR_NONE on success.
     */
    ble_error_t initializeSecurity(bool							 clean_flag	   = false,
...
inline ble_error_t 
BLEDevice::disconnect(Gap::Handle_t connectionHandle, Gap::DisconnectionReason_t reason) {
        return transport->getGap().disconnect(connectionHandle, reason);
}
...
inline ble_error_t
BLEDevice::initializeSecurity(bool							clean_flag,
							  bool                          enableBonding,
                              bool                          requireMITM,
                              Gap::SecurityIOCapabilities_t iocaps,
                              const Gap::Passkey_t          passkey)
{
    return transport->initializeSecurity(clean_flag, enableBonding, requireMITM, iocaps, passkey);
}
