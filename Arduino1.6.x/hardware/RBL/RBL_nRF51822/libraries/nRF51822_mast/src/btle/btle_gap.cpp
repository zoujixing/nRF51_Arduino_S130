...
error_t btle_gap_init(void)
{
    ...
    APP_ERROR_CHECK( sd_ble_gap_device_name_set(&sec_mode, (const uint8_t *) CFG_GAP_LOCAL_NAME, strlen(CFG_GAP_LOCAL_NAME)));
    APP_ERROR_CHECK( sd_ble_gap_appearance_set(CFG_GAP_APPEARANCE));
    APP_ERROR_CHECK( sd_ble_gap_ppcp_set(&gap_conn_params));
    APP_ERROR_CHECK( sd_ble_gap_tx_power_set(CFG_BLE_TX_POWER_LEVEL));
    ...
    cp_init.first_conn_params_update_delay = CFG_FIRST_UPDATE_DELAY;
    cp_init.next_conn_params_update_delay  = CFG_NEXT_UPDATE_DELAY;
    cp_init.max_conn_params_update_count   = CFG_MAX_UPDATE_COUNT;
