...
#include "btle/common.h"
#include "nordic_sdk/ble_advdata.h"
#include "ble_hci.h"
...
ble_error_t nRF51Gap::setAdvertisingData(const GapAdvertisingData &advData, const GapAdvertisingData &scanResponse)
{
    ...
    APP_ERROR_CHECK( sd_ble_gap_adv_data_set(advData.getPayload(),
    ...
    APP_ERROR_CHECK( sd_ble_gap_appearance_set(advData.getAppearance()) );
...
ble_error_t nRF51Gap::startAdvertising(const GapAdvertisingParams &params)
{
    ...
    APP_ERROR_CHECK( sd_ble_gap_adv_start(&adv_para) );
...
ble_error_t nRF51Gap::disconnect(Handle_t connectionHandle, DisconnectionReason_t reason)
{
    ...
    APP_ERROR_CHECK(sd_ble_gap_disconnect(connectionHandle, code));
                              ...
                              const GapScanningParams  *scanParamsIn)
{
    ...
    if (connectionParams != NULL) {
        ...
        connParams.min_conn_interval = CFG_CLIENT_CONNECTION_MIN_INTERVAL_MS;
        connParams.max_conn_interval = CFG_CLIENT_CONNECTION_MAX_INTERVAL_MS;
        connParams.slave_latency     = CFG_CLIENT_SLAVE_LATENCY;
        connParams.conn_sup_timeout  = CFG_CLIENT_SUP_TIMEOUT;
    }
    ...
    if (scanParamsIn != NULL) {
        ...
        scanParams.interval    = CFG_CLIENT_SCAN_INTERVAL; /**< Scan interval between 0x0004 and 0x4000 in 0.625ms units (2.5ms to 10.24s). */
        scanParams.window      = CFG_CLIENT_SCAN_WINDOW;   /**< Scan window between 0x0004 and 0x4000 in 0.625ms units (2.5ms to 10.24s). */
        scanParams.timeout     = CFG_CLIENT_SCAN_TIMEOUT;  /**< Scan timeout between 0x0001 and 0xFFFF in seconds, 0x0000 disables timeout. */
    }
...
{
    APP_ERROR_CHECK( sd_ble_gap_ppcp_get(reinterpret_cast<ble_gap_conn_params_t *>(params)) );
...
{
    APP_ERROR_CHECK( sd_ble_gap_ppcp_set(reinterpret_cast<const ble_gap_conn_params_t *>(params)) );
...
ble_error_t nRF51Gap::setAddress(addr_type_t type, const address_t address)
{
    ...
    APP_ERROR_CHECK( sd_ble_gap_address_set(BLE_GAP_ADDR_CYCLE_MODE_NONE, &dev_addr) );
