...
ble_error_t nRF51GattServer::addService(GattService &service)
{
    ...
    for (uint8_t i = 0; i < service.getCharacteristicCount(); i++) {
        ...
        APP_ERROR_CHECK ( custom_add_in_characteristic(	BLE_GATT_HANDLE_INVALID,
														&nordicUUID,
        ...
        for (uint8_t j = 0; j < p_char->getDescriptorCount(); j++) {
            ...
            APP_ERROR_CHECK( custom_add_in_descriptor(	BLE_GATT_HANDLE_INVALID,
														&nordicUUID,
														p_desc->getValuePtr(),
...
ble_error_t nRF51GattServer::updateValue(Gap::Handle_t connectionHandle, GattAttribute::Handle_t attributeHandle, const uint8_t buffer[], uint16_t len, bool localOnly)
{
    ...
    if (localOnly) {
        ...
        APP_ERROR_CHECK( sd_ble_gatts_value_set(connectionHandle, nrfCharacteristicHandles[attributeHandle].value_handle, &value) );
        return BLE_ERROR_NONE;
    }
            ...
            switch (eventType) {
                case GattServerEvents::GATT_EVENT_DATA_WRITTEN: {
                    GattCharacteristicWriteCBParams cbParams = {
                        ...
                        .handle 	= i,
                        .writeOp 	= static_cast<GattCharacteristicWriteCBParams::WriteOp_t>(gattsEventP->params.write.op),
                ...
                case GattServerEvents::GATT_EVENT_WRITE_AUTHORIZATION_REQ: {
                    ...
                    if (reply.params.write.gatt_status == BLE_GATT_STATUS_SUCCESS) {
                        GattCharacteristicWriteCBParams cbParams = {
							.connHandle = gattsEventP->conn_handle,
                            .handle 	= i,
                            .writeOp 	= static_cast<GattCharacteristicWriteCBParams::WriteOp_t>(gattsEventP->params.authorize_request.request.write.op),
                            .offset     = gattsEventP->params.authorize_request.request.write.offset,
                            .len        = gattsEventP->params.authorize_request.request.write.len,
                            .data       = gattsEventP->params.authorize_request.request.write.data,
                        };
